...
def get_scaled_font(font_size):
    ...
    if not os.path.exists("msjh.ttf"):
        try: urllib.request.urlretrieve("[https://github.com/hanyuan-font/msjh/raw/master/msjh.ttf](https://github.com/hanyuan-font/msjh/raw/master/msjh.ttf)", "msjh.ttf")
        except: pass
        ...
        for col in range(GRID_COLS):
            ...
            if cell_date.strftime("%Y-%m-%d") in events:
                ...
                for time_prefix, event_title, is_holiday_event in events[cell_date.strftime("%Y-%m-%d")]:
                    event_color = COLOR_RED if is_holiday_event else (COLOR_ORANGE if "[SH]" in event_title else (COLOR_BLACK if cell_date.month == month else COLOR_GRAY))
                    ...
                    for text_line, should_indent in lines_info:
                        ...
                        draw.text((x1 + 15 + (time_width if should_indent else 0), y_offset), text_line, fill=event_color, font=font_event)
                        y_offset += 42
    image.save(filename)

# ==================== 3. 主循環 ====================
API_KEY = 'AIzaSyAYBpOB6UoMYeAAmwTM_1KdYEzwtv6zXiE'
CALENDAR_IDS = ['dcyt122024@gmail.com', 'zh.hong_kong#holiday@group.v.calendar.google.com']
...
for i in range(6):
    ...
    events, holiday_dates = {}, set() 
    for cal_id in CALENDAR_IDS:
        try:
            url = f"[https://www.googleapis.com/calendar/v3/calendars/](https://www.googleapis.com/calendar/v3/calendars/){urllib.parse.quote(cal_id)}/events?key={API_KEY}&timeMin={start_date.strftime('%Y-%m-%dT00:00:00Z')}&timeMax={end_date.strftime('%Y-%m-%dT00:00:00Z')}&singleEvents=true&orderBy=startTime&maxResults=250"
            with urllib.request.urlopen(url) as response:
                ...
                for ev in data.get('items', []):
                    ...
                    time_str = f"{start} " if 'dateTime' in ev.get('start', {}) else ""
